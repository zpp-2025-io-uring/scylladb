...
To start the scylla server proper, simply invoke as: scylla server (or just scylla).
    ...
    app_cfg.reactor_opts.max_networking_io_control_blocks.set_default_value(50000);
    {
        ...
        if (std::ranges::contains(candidates, "asymmetric_io_uring")) {
            app_cfg.reactor_opts.reactor_backend.select_default_candidate("asymmetric_io_uring");
        } else if (std::ranges::contains(candidates, "linux-aio")) {
